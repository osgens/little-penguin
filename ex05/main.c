...
static ssize_t ex05_write(struct file *file, const char __user *buf, size_t count, loff_t *pos)
{
	...
	//check input length // Copy 9 bytes from user
	// Check if the written value matches the student login
	if ((count != STUDENT_LOGIN_LEN + 1) || (copy_from_user(temp, buf, STUDENT_LOGIN_LEN)) ||
